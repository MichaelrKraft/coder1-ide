...
class MoltbotBridgeService extends events_1.EventEmitter {
    ...
    handleAgentEvent(payload) {
        ...
        if (!runId)
            return;
        // Log ALL agent event data to understand the protocol
        console.log(`[MoltbotBridge] Agent event: runId=${runId}, stream=${payload.stream}, data=${JSON.stringify(payload.data || {}).substring(0, 500)}`);
        // Track session key
        ...
        const resp = this.agentResponses.get(runId);
        // Accumulate streamed content from various stream types
        ...
        if (payload.stream === 'text' && payload.data) {
            ...
            if (text) {
                resp.content += text;
                console.log(`[MoltbotBridge] Text stream: accumulated ${resp.content.length} chars`);
            }
        }
        ...
        if (payload.data?.content || payload.data?.text) {
            ...
            if (text && !resp.content.includes(text)) {
                resp.content += text;
                console.log(`[MoltbotBridge] Data content: accumulated ${resp.content.length} chars`);
            }
        }
                            ...
                            try {
                                ...
                                if (parsed.type === 'res' && parsed.id === previewId) {
                                    ...
                                    if (parsed.ok && parsed.payload) {
                                        // Extract content from preview response
                                        const p = parsed.payload;
                                        // Check for lastMessage or messages
                                        if (p.lastMessage) {
                                            const content = typeof p.lastMessage === 'string'
                                                ? p.lastMessage
                                                : (p.lastMessage.content || p.lastMessage.text || '');
                                            if (content) {
                                                resolve(typeof content === 'string' ? content :
                                                    (Array.isArray(content) ? content.map((c) => c.text || c).join('') : JSON.stringify(content)));
                                                return;
                                            }
                                        }
                                        if (p.messages && Array.isArray(p.messages)) {
                                            const lastMsg = [...p.messages].reverse().find((m) => m.role === 'assistant');
                                            if (lastMsg?.content) {
