...
Respond with clear file paths and complete file contents in code blocks.`;
  ...
  private async processAgentResponse(
    ...
    for (const block of codeBlocks) {
      if (block.filename && block.content) {
        ...
        // Add to artifacts inbox for user review
        this.addFileToArtifactsInbox(file, agent, team);
      }
    }
  ...
  /**
   * Add a generated file to the artifacts inbox
   */
  private addFileToArtifactsInbox(file: GeneratedFile, agent: AgentSession, team: TeamSession): void {
    try {
      // Map GeneratedFile type to ArtifactType
      const artifactType = this.mapToArtifactType(file.type, file.path);
      ...
      // Add to artifacts inbox with content
      artifactsService.addToInbox({
        ...
        metadata: {
          ...
          description: `Generated by ${agent.agentName} for: ${team.projectRequirement.slice(0, 100)}`,
      ...
      logger.error(`❌ Failed to add artifact to inbox: ${file.path}`, error);
    }
  }
   ...
   * Map GeneratedFile type to ArtifactType
   */
  private mapToArtifactType(fileType: GeneratedFile['type'], filename: string): ArtifactType {
