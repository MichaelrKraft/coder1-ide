const { generateFrontendPrompt } = require('./frontend-developer-template');
const { generateBackendPrompt } = require('./backend-developer-template');
const { generateTestingPrompt } = require('./testing-engineer-template');
...
class PromptGenerator {
   ...
   */
  generate(role, requirements, agentContext) {
   ...
   */
  generateStylingPrompt(requirements, agent) {
...
Use the Write tool to create files directly.`;
   ...
   */
  generateDocsPrompt(requirements, agent) {
    const featureList = requirements.features
      .map((f, i) => `${i + 1}. ${f}`)
      .join('\n   ');
...
Use the Write tool to create documentation files.`;
   ...
   */
  generateGenericPrompt(role, requirements, agent) {
    const featureList = requirements.features
      .map((f, i) => `${i + 1}. ${f}`)
      .join('\n   ');
...
Focus on quality, clarity, and following best practices for ${role} work.`;
   ...
   */
  getPromptLength(role, requirements, agentContext) {
...
module.exports = { PromptGenerator, getPromptGenerator };
