...
import { ArrowLeft, Clock, FileEdit, Terminal, Save, AlertCircle, RefreshCw, Download, History, X, Edit3, Zap, Film, BookOpen } from 'lucide-react';
...
const NotesPanel = nextDynamic(() => import('@/components/notes/NotesPanel'), { ssr: false });
const NoteDetailView = nextDynamic(() => import('@/components/notes/NoteDetailView'), { ssr: false });
...
function TimelineContent() {
  ...
  const activeTab = (searchParams.get('tab') ?? 'timeline') as 'timeline' | 'flowtrace' | 'recordings' | 'notes';
          ...
          <div style={{ height: 'calc(100vh - 180px)' }} className="flex overflow-hidden rounded-lg border border-[#1e1e1e]">
            {activeNotePath ? (
              ...
              <div className="flex-1 overflow-hidden">
                <NotesPanel onNoteSelect={setActiveNotePath} activeNotePath={activeNotePath ?? undefined} />
              </div>
            )}
          </div>
        )}
