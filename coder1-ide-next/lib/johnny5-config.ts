...
export function validateApiKeyFormat(key: string): boolean {
  ...
  // Also support generic API keys (for flexibility)
...
export function setApiKey(key: string): void {
  ...
  logger.debug('[Johnny5] API key saved (encrypted)');
}
...
export function getApiKey(): string | null {
  ...
  try {
    ...
    logger.error('[Johnny5] Failed to decrypt API key:', error);
    return null;
  }
}
...
export function clearApiKey(): void {
  ...
  delete config.apiKeyValidatedAt;
  saveConfigInternal(config);
  logger.debug('[Johnny5] API key cleared');
}
