...
// ============================================================================
class Coder1Client {
    constructor(apiUrl = 'http://localhost:3005') {
        this.apiUrl = apiUrl;
    }
...
function activate(context) {
    ...
    (async () => {
        ...
        if (!isHealthy) {
            vscode.window.showWarningMessage('Coder1 Context API not running. Start server: cd coder1-context-api && npm run dev', 'OK');
        }
        else {
            updateMemoryCount();
        }
    ...
    // COMMAND: Save Memory
    // ============================================================================
    const saveMemoryCommand = vscode.commands.registerCommand('coder1.saveMemory', async () => {
        // Get title from user
        ...
        try {
            ...
            // Show success
            vscode.window.showInformationMessage(`💾 Memory saved! ID: ${memory.id.slice(0, 12)}...`);
            // Update count
            updateMemoryCount();
        }
        catch (error) {
            vscode.window.showErrorMessage(`Failed to save memory: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    });
    ...
    // ============================================================================
    const searchMemoryCommand = vscode.commands.registerCommand('coder1.searchMemory', async () => {
        // Get search query
        ...
        try {
            ...
            const selected = await vscode.window.showQuickPick(results.map((memory) => ({
            ...
            if (selected) {
                ...
                const action = await vscode.window.showInformationMessage(details.join('\n'), 'Copy ID', 'Close');
                if (action === 'Copy ID') {
    ...
    // ============================================================================
    const viewAllCommand = vscode.commands.registerCommand('coder1.viewAll', async () => {
        try {
            ...
            const selected = await vscode.window.showQuickPick(memories.map((memory) => ({
                label: `$(bookmark) ${memory.content.title}`,
    ...
    // ============================================================================
    const checkStatusCommand = vscode.commands.registerCommand('coder1.checkStatus', async () => {
        const isHealthy = await client.checkHealth();
        if (isHealthy) {
            const { count } = await client.listMemories();
            vscode.window.showInformationMessage(`✅ Coder1 Context API is running!\n${count} memories saved.`);
            updateMemoryCount();
        }
        else {
            vscode.window.showWarningMessage('❌ Coder1 Context API is not running.\n\nStart it with:\ncd coder1-context-api && npm run dev', 'Copy Command').then(action => {
    ...
    // ============================================================================
    const saveSessionCommand = vscode.commands.registerCommand('coder1.saveSession', async () => {
        try {
            ...
            vscode.window.showInformationMessage(`🎬 Session saved! Captured ${allFiles.length} files, ${totalLines} lines${gitContext ? `, Git: ${gitContext.branch}` : ''}`);
            updateMemoryCount();
        }
        catch (error) {
            vscode.window.showErrorMessage(`Failed to save session: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    });
