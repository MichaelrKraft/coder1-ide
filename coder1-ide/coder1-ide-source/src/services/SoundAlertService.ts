...
export class SoundAlertService {
   ...
   */
  private getPresetConfig(preset: SoundPreset): { frequency: number; duration: number; volume: number; waveType?: OscillatorType } {
    switch (preset) {
      case 'gentle':
  ...
  /**
   * Get appropriate frequency for different alert types (legacy support)
   */
  private getFrequencyForType(type: SoundOptions['type']): number {
    switch (type) {
      case 'success':
      case 'completion':
        return 800; // Pleasant notification tone
      case 'error':
        return 400; // Lower, more urgent tone
      case 'notification':
        return 600; // Mid-range attention tone
      default:
        return 800;
    }
  }
  ...
  /**
   * Play specific alert types with optimized settings
   */
  ...
  async playSuccessAlert(): Promise<void> {
    return this.playAlert({
      type: 'success',
      frequency: 900,
      duration: 0.3,
  ...
  async playNotificationAlert(): Promise<void> {
    return this.playAlert({
      type: 'notification',
      frequency: 600,
      duration: 0.4,
