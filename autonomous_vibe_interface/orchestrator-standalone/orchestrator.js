...
class OrchestratorApp {
    constructor() {
        ...
        this.phases = [
            ...
            { key: 'team', label: 'Team Assembly', description: 'Assembles 3+ expert AI agents' },
    ...
    init() {
        console.log('[Orchestrator] Initializing standalone application');
    ...
    setupVoiceRecognition() {
        if (!('webkitSpeechRecognition' in window) && !('SpeechRecognition' in window)) {
            console.warn('[Orchestrator] Speech recognition not supported');
            return;
        }
        ...
        this.recognition.onstart = () => {
            console.log('[Orchestrator] Voice recording started');
        ...
        this.recognition.onend = () => {
            console.log('[Orchestrator] Voice recording ended');
            this.isRecording = false;
            const micBtn = document.getElementById('microphone-btn');
            if (micBtn) {
        ...
        this.recognition.onerror = (event) => {
            console.error('[Orchestrator] Voice recognition error:', event.error);
            this.isRecording = false;
            const micBtn = document.getElementById('microphone-btn');
            if (micBtn) {
                micBtn.classList.remove('recording', 'processing');
    ...
    startVoiceRecording() {
        if (!this.recognition) {
            console.warn('[Orchestrator] Speech recognition not available');
            return;
        }
        ...
        if (this.isRecording) {
            ...
            try {
                ...
                console.error('[Orchestrator] Failed to start voice recognition:', error);
            }
        }
    }
    ...
    connectSocket() {
        if (typeof io === 'undefined') {
            console.warn('[Orchestrator] Socket.IO not available, running in demo mode');
            return;
        }
        
        const socketUrl = 'http://localhost:3000';
        console.log('[Orchestrator] Connecting to:', socketUrl);
    ...
    setupSocketHandlers() {
        this.socket.on('connect', () => {
            console.log('[Orchestrator] Connected:', this.socket.id);
        });
        ...
        this.socket.on('conversation:started', (data) => {
            console.log('[Orchestrator] Conversation started:', data);
            this.session = {
        ...
        this.socket.on('conversation:phase-changed', (data) => {
            console.log('[Orchestrator] Phase changed:', data);
            if (this.session) {
        ...
        this.socket.on('conversation:phase-change', (data) => {
            console.log('[Orchestrator] Phase change event:', data);
            if (this.session) {
        ...
        this.socket.on('conversation:message', (data) => {
            console.log('[Orchestrator] Message received:', data);
            this.addMessage({
        ...
        this.socket.on('conversation:orchestrator-message', (data) => {
            console.log('[Orchestrator] Orchestrator message received:', data);
            this.addMessage({
        ...
        this.socket.on('conversation:agent-message', (data) => {
            console.log('[Orchestrator] Agent message received:', data);
        ...
        this.socket.on('conversation:expert-thinking', (data) => {
            console.log('[Orchestrator] Expert thinking:', data);
        ...
        this.socket.on('conversation:plans-received', (data) => {
            console.log('[Orchestrator] Plans received:', data);
            if (this.session) {
        ...
        this.socket.on('conversation:synthesis-complete', (data) => {
            console.log('[Orchestrator] Synthesis complete:', data);
            if (this.session) {
        ...
        this.socket.on('conversation:complete', (data) => {
            console.log('[Orchestrator] Conversation complete:', data);
            if (this.session) {
        ...
        this.socket.on('conversation:error', (data) => {
            console.error('[Orchestrator] Conversation error:', data);
            this.isLoading = false;
            this.addMessage({
                id: `error-${Date.now()}`,
                agent: 'System',
    ...
    showConversationScreen() {
        ...
        console.log('[Orchestrator] Conversation screen displayed, interaction area visible');
    }
    ...
    handleStreamChunk(data) {
        ...
        if (!streamingMessage) {
            console.warn('[Streaming] No streaming message found for chunk:', messageId);
            return;
        }
    ...
    handleStreamComplete(data) {
        ...
        if (!streamingMessage) {
            console.warn('[Streaming] No streaming message found for completion:', messageId);
            return;
        }
    ...
    retryStreamingResponse(originalData) {
        if (!this.socket || !this.session?.sessionId) {
            console.warn('[Stream Retry] No socket or session available');
            return;
        }
...
function startConsultation() {
    ...
    if (!userQuery || !app.socket) {
        console.warn('[Orchestrator] No query or socket not connected');
        return;
    }
...
function sendUserMessage() {
    ...
    console.log('[Orchestrator] Sending user message:', userInput);
...
function generateClaudeCodePrompt() {
    ...
    if (!app.socket || !app.session?.sessionId) {
        console.warn('[Orchestrator] No socket connection or session');
        return;
    }
    ...
    if (!app.session?.synthesisComplete) {
        console.warn('[Orchestrator] Synthesis not complete yet');
    ...
    console.log('[Orchestrator] Generating Claude Code prompt');
...
function exportPlan() {
    ...
    if (!app.session) {
        console.warn('[Orchestrator] No session available for export');
...
class ConsultationTimer {
    ...
    start() {
        ...
        console.log('[Timer] Starting 3-minute consultation countdown');
        this.isRunning = true;
        this.showTimer();
        this.updateDisplay();
    ...
    handleOvertime() {
        console.log('[Timer] Consultation going into overtime');
    ...
    complete() {
        ...
        console.log('[Timer] Consultation completed!');
...
function toggleTheme() {
    ...
    if (isLightTheme) {
        ...
        console.log('[Theme] Switched to dark theme');
    } else {
        // Switch to light theme
        document.body.classList.add('light-theme');
        localStorage.setItem('orchestrator-theme', 'light');
        console.log('[Theme] Switched to light theme');
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    script.onload = () => {
        console.log('[Orchestrator] Socket.IO loaded, initializing app');
        app = new OrchestratorApp();
    };
    script.onerror = () => {
        console.error('[Orchestrator] Failed to load Socket.IO');
        // Initialize app anyway for UI functionality (without real-time features)
        console.log('[Orchestrator] Initializing app without Socket.IO');
        app = new OrchestratorApp();
    };
