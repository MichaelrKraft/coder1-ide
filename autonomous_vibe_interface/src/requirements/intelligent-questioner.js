...
class IntelligentQuestioner {
    ...
    async analyzeUserRequest(userRequest) {
        try {
            // Use Claude Code SDK if available, otherwise fallback to template analysis
            if (this.claudeSDK && this.claudeAPIKey) {
                this.logger.info('Using Claude Code SDK for request analysis');
                return await this.claudeSDK.analyzeRequest(this.claudeAPIKey, userRequest);
            } else {
