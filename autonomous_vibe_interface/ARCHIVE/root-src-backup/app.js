...
// Import Hivemind service
const HivemindService = require('./services/HivemindService');
// Note: ClaudeInfiniteAPI uses ES modules and will be instantiated separately when needed

// Hivemind API Routes
app.post('/api/hivemind/start', async (req, res) => {
    try {
        const { userId, projectBrief, taskDescription } = req.body;
        ...
        // Start hivemind session
        const result = await HivemindService.startSession(
            userId, 
            projectBrief || 'Hivemind collaborative generation', 
            taskDescription
        );
        
        // Note: ClaudeAPI hivemind mode will be enabled when ClaudeInfiniteAPI is used
        ...
        console.error('Error starting hivemind:', error);
        res.status(500).json({ error: error.message });
    }
});

app.get('/api/hivemind/status/:sessionId', (req, res) => {
    try {
        const { sessionId } = req.params;
        const status = HivemindService.getSessionStatus(sessionId);
        ...
        console.error('Error getting hivemind status:', error);
        res.status(500).json({ error: error.message });
    }
});

app.post('/api/hivemind/assign-task', (req, res) => {
    try {
        const { sessionId, agentId, task } = req.body;
        
        if (!sessionId || agentId === undefined || !task) {
            return res.status(400).json({ 
                error: 'Missing required fields: sessionId, agentId, task' 
            });
        }
        
        HivemindService.assignTask(sessionId, agentId, task);
...
app.get('/api/hivemind/shared-context/:sessionId', (req, res) => {
    try {
        const { sessionId } = req.params;
        const { agentId } = req.query;
        
        if (agentId !== undefined) {
            const context = HivemindService.getAgentContext(sessionId, parseInt(agentId));
            res.json(context);
        } else {
            const memory = HivemindService.agentMemory.get(sessionId);
            if (!memory) {
                return res.status(404).json({ error: 'Session not found' });
            }
            res.json(memory);
...
app.post('/api/hivemind/stop/:sessionId', (req, res) => {
    try {
        const { sessionId } = req.params;
        const metrics = HivemindService.stopSession(sessionId);
        
        // Note: ClaudeAPI hivemind mode will be disabled when session ends
        
        res.json({ success: true, metrics });
    } catch (error) {
        console.error('Error stopping hivemind:', error);
        res.status(500).json({ error: error.message });
    }
});
...
io.on('connection', (socket) => {
    ...
    // Hivemind WebSocket events
    socket.on('hivemind:join_session', (data) => {
        const { sessionId } = data;
        socket.join(`hivemind:${sessionId}`);
        socket.hivemindSessionId = sessionId;
        
        const status = HivemindService.getSessionStatus(sessionId);
        if (status) {
            socket.emit('hivemind:session_joined', { status });
        }
    });
    
    socket.on('hivemind:leave_session', (data) => {
        const { sessionId } = data;
        socket.leave(`hivemind:${sessionId}`);
        delete socket.hivemindSessionId;
    });
});

// Set up Hivemind event listeners for real-time updates
HivemindService.on('session:started', (data) => {
    io.to(`hivemind:${data.sessionId}`).emit('hivemind:session_started', data);
});

HivemindService.on('discovery:added', (data) => {
    io.to(`hivemind:${data.sessionId}`).emit('hivemind:discovery', data);
});

HivemindService.on('task:assigned', (data) => {
    io.to(`hivemind:${data.sessionId}`).emit('hivemind:task_assigned', data);
});

HivemindService.on('progress:updated', (data) => {
    io.to(`hivemind:${data.sessionId}`).emit('hivemind:progress', data);
});

HivemindService.on('task:completed', (data) => {
    io.to(`hivemind:${data.sessionId}`).emit('hivemind:task_completed', data);
});

HivemindService.on('pattern:added', (data) => {
    io.to(`hivemind:${data.sessionId}`).emit('hivemind:pattern', data);
});

HivemindService.on('queen:changed', (data) => {
    io.to(`hivemind:${data.sessionId}`).emit('hivemind:queen_changed', data);
});

HivemindService.on('session:stopped', (data) => {
    io.to(`hivemind:${data.sessionId}`).emit('hivemind:session_stopped', data);
});
...
server.listen(PORT, () => {
    ...
    console.log(`🧠 Hivemind API: http://localhost:${PORT}/api/hivemind/*`);
    console.log(`🔊 Socket.IO: Voice and Hivemind real-time communication enabled`);
});
